...
public class GridManager2 : MonoBehaviour
{
    ...
    public TextMeshProUGUI manaNotification;//thong bao cua game

    [Header("EFFECT")]
    public GameObject missilePrefab;

    [HideInInspector] public int shotsRemaining = 1; // Mặc định 1 lượt bắn mỗi turn
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                try
                {
                    ...
                    gridCellComponent.Initialize(x, y, this); // THIS phải là GridManager2 
    ...
    {
        Debug.Log("▶️ Player 2 StartTurn called");

        turn++;
        shotsRemaining = 1;
        mana = Mathf.Min(mana + 1, maxMana);

        ISkill[] skills = GetComponentsInChildren<ISkill>();
        foreach (ISkill skill in skills)
        {
            skill.TickCooldown();
        }

        bool isMyTurn = TurnManager.Instance.GetCurrentTurn() == gridOwner;

        manaNotification.gameObject.SetActive(isMyTurn);

        if (isMyTurn)
        {
            manaNotification.text = $"Mana {gridOwner}: {mana}/{maxMana}";
        }

        notification.text = $"🔄 Bắt đầu lượt {turn}. Bạn có {shotsRemaining} lượt bắn.";
    }
    ...
    public void HideUI()
    {
        if (manaNotification != null)
            manaNotification.gameObject.SetActive(false);
    }
    ...
    {
        if (!isGameStarted || shotsRemaining <= 0)
        {
            return HitType.Miss;
        }
        ...
        // Nếu trúng → không trừ lượt
        if (hitType == HitType.BodyHit || hitType == HitType.HeadHit)
        {
            notification.text += "\n🎯 Trúng mục tiêu! Bạn được bắn tiếp!";
            return hitType;
        }

        // Nếu không trúng → trừ lượt
        shotsRemaining--;
        if (shotsRemaining <= 0)
        {
            notification.text += "\n⚠️ Hết lượt";
